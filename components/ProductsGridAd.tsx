...
function ProductCard({ product }: { product: Product }) {
  return (
    <a
      ...
      className="flex h-full min-w-0 flex-col overflow-hidden rounded-xl bg-[#13131a] border border-[#2a2a3a] hover:border-violet-600/50 transition-colors group"
    >
      <div className="h-20 w-full shrink-0 bg-[#1c1c28] flex items-center justify-center p-2">
        {product.image ? (
...
export default async function ProductsGridAd({ className = "" }: Props) {
  ...
  return (
    <div
      className={`bg-[#13131a] rounded-xl border border-[#2a2a3a] p-5 ${className}`}
    >
